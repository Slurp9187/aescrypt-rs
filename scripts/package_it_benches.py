"""
package_it.py – FINAL VERSION (include_files on all packages)

Features:
- All 4 packages generated with timestamp prefix
- `include_files` field present on every package (defaults to [])
- Currently only used in `mod` package to force-include src/lib.rs
- Ready for future additions (e.g. benches, examples, etc.)
"""
...
PACKAGES = [
    {
        ...
        "file_pattern": "*.rs",
        "include_files": [],  # ← ready for future use
        "description": "Full benches source + Cargo.toml",
    },
    # {
    #     "suffix": "tests",
    #     "root_files": [],
    ...
    #     "file_pattern": "mod.rs",
    #     "include_files": [
    #         "src/lib.rs"                     # ← only this one, always first
    #     ],
    #     "description": "Module structure overview – lib.rs + all mod.rs files",
    # },
]
...
def create_package(pkg_def: dict, root: Path, timestamp: str) -> None:
    ...
    # 2. Explicit include_files (e.g. src/lib.rs)
    for path_str in pkg_def.get("include_files", []):
        p = root / path_str
        if p.is_file():
            rel = p.relative_to(root).as_posix()
            if rel not in seen_paths:
...
def package_all(root: Path = PROJECT_ROOT) -> None:
    ...
    print("Done! `include_files` is now available on every package.")
